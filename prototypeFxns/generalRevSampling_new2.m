function Rev = generalRevSampling_new2(Omega,DGr_RT,numSamples,thinning,burn_in,xcurr)
...
% Get the RHS sense (i.e., DGr/RT sign)
sense  = sign(DGr_RT);
DGr_RT = -abs(DGr_RT);      % force DGr_RT in the forward direction  
...
% Return actual reversibilities (not log) with the appropriate sense
Dsense = diag(sum(Omega.*repmat(-sense,1,size(Omega,2)),1));
Rev    = exp(Dsense*Rev);