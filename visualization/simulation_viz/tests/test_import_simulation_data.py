...
class TestImportSimulationData(unittest.TestCase):
    ...
    def test_gather_sim_data(self):
        ...
        self.assertTrue(pd.DataFrame.equals(true_res_conc_interp, conc))
        self.assertTrue(pd.DataFrame.equals(true_res_flux_interp, flux))
    ...
    def test_get_time_series_quantiles_abs(self):
        ...
        conc, flux = gather_sim_data(mat, met_names, rxn_names, n_models,
                                     ref_conc_dic=ref_conc_dic, ref_flux_dic=ref_flux_dic)

        data_type = 'conc_abs'
        conc_quantiles_df = get_time_series_quantiles(conc, data_type, met_names)

        data_type = 'flux_abs'
        flux_quantiles_df = get_time_series_quantiles(flux, data_type, rxn_names)

        self.assertTrue(pd.DataFrame.equals(true_res_conc, conc_quantiles_df))
        self.assertTrue(pd.DataFrame.equals(true_res_flux, flux_quantiles_df))
    ...
    def test_load_simulations(self):
        ...
        conc, flux, met_names, rxn_names = load_simulation(self.raw_data_dir, self.model_name,
