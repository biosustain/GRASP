...
def plot_model_interactive(data_df: pd.DataFrame, data_interp_df: pd.DataFrame, model_i: int, quant_type: str,
    ...
    if quant_type in ['conc', 'conc_unscaled', 'flux']:
        ...
        plot1 = alt.Chart(data_interp_df[(data_interp_df['model'] == model_i) & (data_interp_df[data_type].isin(selected_data))]).mark_point().encode(
            alt.X('time_point:Q',
                  scale=alt.Scale(domain=x_lim, type=x_scale) if x_lim else alt.Scale(type=x_scale)
                  ),
