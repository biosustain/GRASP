...
if ~all(size(rxnsList) == [nRxns+1, 5])
    error(['Check the rxns sheet, it should have ', num2str(nRxns+1), ' rows and 5 columns.', ...
           newline, ...
           'Columns it should have: ID, reaction name, transportRxn?, modelled?, isoenzymes.', ...
           newline, ...
...
if ~all(size(metsList) == [nMets+1, 6])
    error(['Check the mets sheet, it should have ', num2str(nMets+1), ' rows and 6 columns.', ...
           newline, ...
           'Columns it should have: ID, Metabolite name, balanced?, active?, fixed?, measured?',...
           newline, ...
           'For a more detailed input validation please use the set_up_grasp_models package.']);
end

nMetsActive = size(find(xMets(:, 2)==1), 1);
    
if size(idxMeas, 2) ~= 5
    error(['Check the measRates sheet, it should have 5 columns.', ...
          newline, ...
          'Columns it should have: Fluxes (umol/gCDW/h), MBo10_mean, MBo10_std, MBo10_mean2, MBo10_std2.',...
           newline, ...
           'For a more detailed input validation please use the set_up_grasp_models package.']);
end
...
if ~all(size(xDG_std) == [nRxns, 2])
    error(['Check the thermoRxns sheet, it should have ', num2str(nRxns), ' rows and 3 columns.', ...
           newline, ...
           'Columns it should have: rxn, ∆Gr''_min (kJ/mol), ∆Gr''_max (kJ/mol)',...
           newline, ...
           'For a more detailed input validation please use the set_up_grasp_models package.']);
end
...
if ~all(size(xMetsThermo) == [nMets, 2])
    error(['Check the thermoMets sheet, it should have ', num2str(nMets), ' rows and 3 columns.', ...
          newline, ...
          'Columns it should have: met, min (M), max (M)',...
           newline, ...
           'For a more detailed input validation please use the set_up_grasp_models package.']);
end

if ~all(size(idxProt) == [nRxnsActive+1, 4])
    error(['Check the protData sheet, it should have ', num2str(nRxnsActive+1), ' rows and 4 columns.', ...
          newline, ...
          'Columns it should have: enzyme/rxn, MBo10_LB2, MBo10_meas2, MBo10_UB2',...
           newline, ...
           'For a more detailed input validation please use the set_up_grasp_models package.']);
end

if ~all(size(idxMets) == [nMetsActive+1, 4])
    error(['Check the metsData sheet, it should have ', num2str(nMetsActive), ' rows and 4 columns.', ...
           newline, ...
           'Columns it should have: met, MBo10_LB2, MBo10_meas2, MBo10_UB2.',...
           newline, ...
           'For a more detailed input validation please use the set_up_grasp_models package.']);
end
