...
if size(idxMeas, 2) < 3
    error(['Check the measRates sheet, it should have at least 3 columns.', ...
          newline, ...
          'Columns it should have: reaction ID, vref_mean (umol/gCDW/h), vref_std (umol/gCDW/h).',...
           newline, ...
           'For a more detailed input validation please use the set_up_grasp_models package.']);
end
...
if size(idxMets,1) ~= nMets+1 || size(idxMets,2) < 4
    error(['Check the metsData sheet, it should have ', num2str(nMets), ' rows and 4 columns.', ...
           newline, ...
