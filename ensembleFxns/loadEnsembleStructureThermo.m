...
if ~isempty(poolConstraints)
    ensemble.poolConst = poolConstraints';
end
...
[gibbsRanges,logX_ranges] = calculateGibbsEnergyRanges(Sthermo,xDG_std,xMetsThermo,dirFluxRef(idxNotExch),ineqConstraints')
return
...
ensemble.metsDataMin(ensemble.metsDataMin==0)   = 1e-2;                                                % Set bounds on mets without measured data, [lb,ub] ~ [1nM,1mM]
ensemble.metsDataMean(ensemble.metsDataMean==0) = 1e0;
ensemble.metsDataMax(ensemble.metsDataMax==0)   = 1e2;
...
for jx = 1:ensemble.numStruct
    ...
    if (jx==1)
        ...
        ensemble.fixedExch    = [ensemble.refFlux(ensemble.kinInactRxns,:),ensemble.expFluxes(ensemble.kinInactRxns,:)];
